...
public class PromotionAction extends BaseAction<Promotion> {
        ...
        public String findMainPromotions() {
                ...
                PageBean<Promotion> pageBean = WebClient.create(Constant.MANAGEMENT_URL
                                + "/findPromotionByPage?page="+page+"&rows="+rows)
        ...
        public String promotion_showDetail() {
                ...
                if (!templateFile.exists()) {
                        ...
                        try {
                                configuration.setDirectoryForTemplateLoading(new File("/bos_fore/src/main/webapp/WEB-INF/template"));
                                Template template = configuration.getTemplate("promotion_detail.ftl");
                                // 动态获取促销数据
                                Promotion promotion = WebClient.create(Constant.MANAGEMENT_URL + "/findPromotionById/?id="+model.getId())
                                                .accept(MediaType.APPLICATION_JSON).get(Promotion.class);
                                HashMap<String, Object> map = new HashMap<String, Object>();
                                map.put("promotion", promotion);
                                template.process(map, new PrintWriter(new OutputStreamWriter(new FileOutputStream(templateFile), "utf-8")));
